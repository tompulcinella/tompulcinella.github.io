(async () => {
// Code wasn't working and Google said that it may be because the HTML wasn't fully loading
// I added this to my code and somehow it started working. Don't ask me how, it could be magic for all I know.
// I tried testing it by going back to a previous version we made in class, but ended up confusing myself even further.
// The code works, and that's all that matters to me at this point in time. Sorry.
...
let dropChordQuality = document.getElementById("chord-quality");
//let slider = document.getElementById("slide");

// slider.addEventListener("change", function () {
//   document.getElementById("transpoAmt").innterText = `${slider.value} semitones`;
// })
...
WebMidi.outputs.forEach(function (output, num) {
 ...
 const midiProcess = function (midiNoteInput) {
  ...
  let chordQuality = dropChordQuality.value; // Retrieves chord quality from the dropdown
  let chordIntervals = (chordQuality === "major") ? [0, 4, 7] : [0, 3, 7]; 
  let midiChordOutput = chordIntervals.map(interval => pitch + interval); 
// I made two drafts of this project, one that only made major chords, and this one that does both major and minor.
// However, I though that to be too boring to submit so for this draft I had some assistance from a roommate
// who taught me some new js concepts, hence the use of "===" and "=>", which I don't believe we went over in class.                                                              
  return midiChordOutput;
};
...
})(); //part of the "async" function from the top, please don't ask me to explain exactly what this does, for I still do not know.